...
module.exports = {
  async up(queryInterface, Sequelize) {
    if (!(await tableExists(queryInterface, "attendance_daily_reasons"))) {
    ...
    await addIndexSafe(queryInterface, "attendance_daily_reasons", ["businessId", "employeeId", "dateYmd", "reasonType"], "attendance_daily_reasons_lookup_idx");

    const linksExist = await tableExists(queryInterface, "telegram_account_links");
    const links = linksExist ? await queryInterface.describeTable("telegram_account_links") : {};
    if (linksExist && !links.pendingAction) {
      await queryInterface.addColumn("telegram_account_links", "pendingAction", { type: Sequelize.JSONB, allowNull: true });
    }
  },

  async down(queryInterface) {
    if (await tableExists(queryInterface, "telegram_account_links")) {
