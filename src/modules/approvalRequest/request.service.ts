...
export class RequestService {
  ...
  async actOnRequest(requestId: string, businessId: string, userId: string, payload: any) {
    ...
    if (payload.action === 'reject') {
      ...
      await InternalNotifier.send({ businessId, recipientUserId: req.requestedByUserId, senderUserId: userId, moduleKey: 'approval', type: 'approval_rejected', title: 'Request Rejected', message: 'Your approval request was rejected.', entityType: 'approval_request', entityId: req.id });
    } else if (payload.action === 'approve') {
      if (step.isFinalStep) {
        await req.update({ status: 'approved', finalDecision: 'approved', completedAt: new Date() });
        await InternalNotifier.send({ businessId, recipientUserId: req.requestedByUserId, senderUserId: userId, moduleKey: 'approval', type: 'approval_approved', title: 'Request Approved', message: 'Your approval request was formally approved.', entityType: 'approval_request', entityId: req.id });
      } else {
      ...
      await InternalNotifier.send({ businessId, recipientUserId: req.requestedByUserId, senderUserId: userId, moduleKey: 'approval', type: 'approval_returned', title: 'Request Returned', message: 'Your approval request was returned for modifications.', entityType: 'approval_request', entityId: req.id });
