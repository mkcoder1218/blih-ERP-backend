...
const VAGUE_REASON_PATTERN =
  /^(late|traffic|personal|emergency|issue|n\/a|na|none|other|misc|because)$/i;
...
function buildNoticeDeadline(dateUtc: Date, timeZone: string) {
  ...
  return new Date(
    dayStart.getTime() +
      (LATE_NOTICE_DEADLINE_HOUR * 60 + LATE_NOTICE_DEADLINE_MINUTES) * 60_000,
  );
}
...
export class AttendanceRequestsService {
  ...
  async create(
    ...
    if (!data.title || !data.reason)
      throw new Error("title and reason are required.");
    const isCorrection = data.requestType === "check_in_correction";
    const isLatenessNotice = data.requestType === "lateness_notice";
    const isWorkFromHome = data.requestType === "work_from_home";
    const canTargetEmployee =
      isCorrection || (isLatenessNotice && options.canManage);
    const targetEmployeeUserId =
      canTargetEmployee && data.employeeUserId
        ? String(data.employeeUserId || "")
        : employeeUserId;
    ...
    if (canTargetEmployee && !targetEmployeeUserId)
      throw new Error("Employee is required.");
    if (isCorrection) {
      if (!targetEmployeeUserId)
        throw new Error(
          "Employee is required for check-in correction requests.",
        );
      ...
      correctionFromAt = parseCorrectionWallTime(
        data.fromAt,
      ...
      const employee = await db.User.findOne({
        where: { id: targetEmployeeUserId, businessId },
      });
    ...
    if (isLatenessNotice) {
      ...
      noticeFromAt = data.fromAt
        ? parseCorrectionWallTime(
            data.fromAt,
            settings?.timezone || "Africa/Addis_Ababa",
          )
        : new Date();
      if (!noticeFromAt)
        throw new Error("Valid lateness notice date and time is required.");
      deadlineAt = buildNoticeDeadline(
        noticeFromAt,
        settings?.timezone || "Africa/Addis_Ababa",
      );
      reasonText = String(data.reasonText || data.reason || "").trim();
      validityStatus = validateNoticeText(reasonText);
      const reasonCode = this.latenessRules.normalizeReasonCode(
        data.reasonCategory || data.category,
      );
      const rule = await this.latenessRules.findRule(businessId, reasonCode);
      if (!rule || rule.enabled === false || rule.isActive === false)
        throw Object.assign(
          new Error("Selected lateness reason is not enabled."),
          { statusCode: 400 },
        );
      if (rule.requiresAttachment && !data.attachmentUrl && !data.attachmentId)
        throw Object.assign(
          new Error("Selected lateness reason requires an attachment."),
          { statusCode: 400 },
        );
      if (new Date() > deadlineAt && !rule.allowAfterDeadline)
        validityStatus = "expired";
      const creditConfig = await this.latenessRules.getCreditConfig(businessId);
      const used =
        creditConfig.mode === "GLOBAL_POOL"
          ? await this.latenessRules.countApprovedUsableGlobal(
              businessId,
              targetEmployeeUserId,
              noticeFromAt,
            )
          : await this.latenessRules.countApprovedUsableByReason(
              businessId,
              targetEmployeeUserId,
              reasonCode,
              noticeFromAt,
      ...
      const limit =
        creditConfig.mode === "GLOBAL_POOL"
          ? creditConfig.globalMonthlyLimit
          : Number(rule.monthlyLimit || 0);
      if (limit <= 0 || used >= limit) {
        const behavior = String(
          (creditConfig.mode === "GLOBAL_POOL"
            ? creditConfig.behaviorWhenExceeded
            : rule.behaviorWhenExceeded) || "HR_REVIEW",
        ).toUpperCase();
        if (behavior === "BLOCK")
          throw Object.assign(
            new Error("Monthly limit reached for this lateness reason."),
            { statusCode: 400 },
          );
    ...
    let wfhFromAt: Date | null = null;
    let wfhToAt: Date | null = null;
    let wfhDurationMinutes: number | null = null;
    let wfhCategory: string | null = null;
    let normalizedReason = String(data.reason || "").trim();

    if (isWorkFromHome) {
      const validated = await this.workFromHomeService.validateCreate({
        businessId,
      ...
      wfhFromAt = validated.fromAt;
      wfhToAt = validated.toAt;
      wfhDurationMinutes = validated.durationMinutes;
      wfhCategory = validated.category;
    ...
    return db.AttendanceRequest.create({
      ...
      category: isLatenessNotice
        ? this.latenessRules.normalizeReasonCode(
            data.reasonCategory || data.category,
          )
        : isWorkFromHome
          ? wfhCategory
          : data.category || data.reasonCategory || null,
      title: data.title,
      ...
      fromAt: isCorrection
        ...
        : isLatenessNotice
          ...
          : isWorkFromHome
            ? wfhFromAt
            : data.fromAt || null,
      toAt: isWorkFromHome ? wfhToAt : data.toAt || null,

      durationMinutes: isWorkFromHome
        ? wfhDurationMinutes
      ...
      reasonCategory: isLatenessNotice
        ? this.latenessRules.normalizeReasonCode(
            data.reasonCategory || data.category,
          )
        : data.reasonCategory || data.category || null,
      reasonText,
      validityStatus,
  ...
  async action(
    ...
    requestId: string,
    userId: string,
    status: "approved" | "rejected" | "invalid" | "expired" | "cancelled",
    note?: string,
  ) {
    if (!VALID_STATUSES.has(status)) throw new Error("Invalid status.");
    const record = await db.AttendanceRequest.findOne({
      where: { id: requestId, businessId },
    });
    if (!record) throw new Error("Attendance request not found.");
    if (record.status !== "pending")
      throw new Error("Only pending attendance requests can be updated.");
    if (
      status === "rejected" &&
      record.requestType === "work_from_home" &&
      String(note || "").trim().length < 3
    ) {
      throw Object.assign(
        new Error(
          "A rejection reason is required for work-from-home requests.",
        ),
        {
          statusCode: 400,
        },
    ...
    if (record.requestType === "lateness_notice" && status === "approved") {
      const validity = await this.evaluateLatenessNotice(
        record,
      ...
      if (validity.validityStatus !== "valid" || !validity.usable) {
        if (
          ...
          await record.update({
            ...
            actionNote: validity.message || note || null,
          });
        }
        throw Object.assign(
          new Error(validity.message || "Lateness notice is invalid"),
          { statusCode: 400 },
        );
      }
    }
    ...
    await record.update({
      status,
      validityStatus:
        record.requestType === "lateness_notice"
      ...
      approvedByUserId:
        status === "approved" ? userId : record.approvedByUserId,
      rejectedAt:
        status === "rejected" || status === "invalid" || status === "expired"
          ? now
          : record.rejectedAt,
      rejectedByUserId:
        status === "rejected" || status === "invalid" || status === "expired"
  ...
  async cancelWorkFromHome(
    businessId: string,
    requestId: string,
    employeeUserId: string,
  ) {
    return this.workFromHomeService.cancelPending(
      businessId,
      requestId,
      employeeUserId,
    );
  ...
  async evaluateLatenessNotice(record: any, lateByMinutes = 0) {
    ...
    if (validityStatus === "invalid")
      return {
        ...
        usable: false,
        reasonCode: null,
        message: "Reason is too vague or too short.",
      };
    return this.latenessRules.evaluateNotice(record, lateByMinutes);
  }

  async countApprovedLatenessNotices(
    businessId: string,
    employeeUserId: string,
    anchorDate: Date,
  ...
  async syncApprovedCorrections(businessId: string, query: any = {}) {
    ...
    for (const record of requests) {
      if (
        !CORRECTION_EVENT_TYPES.has(String(record.category || "")) ||
        !record.fromAt
      )
        continue;
      const correctedAtUtc = new Date(record.fromAt);
