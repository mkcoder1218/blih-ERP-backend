...
export const createCategorySchema = Joi.object({
  ...
  key: Joi.string().trim().max(120).lowercase().optional(),
...
export const listArticlesQuerySchema = Joi.object({
  page: Joi.number().integer().min(1).default(1),
  size: Joi.number().integer().min(1).max(100).default(20),
  search: Joi.string().allow('', null).optional(),
  categoryId: Joi.string().uuid().allow('', null).optional(),
  ...
  includeArchived: Joi.boolean().optional(),
  sortBy: Joi.string().valid('title', 'createdAt', 'updatedAt', 'publishedAt', 'version').default('updatedAt'),
