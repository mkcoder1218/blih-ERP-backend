...
export type LeadModel = ModelStatic<any> & { associate?: (models: any) => void; };

export default (sequelize: Sequelize, dataTypes: typeof DataTypes): LeadModel => {
  ...
  }, { tableName: "crm_leads", timestamps: true, paranoid: true }) as LeadModel;

  Lead.associate = (models: any) => {
    models.Lead.belongsTo(models.Business, { foreignKey: "businessId" });
    if(models.User) models.Lead.belongsTo(models.User, { foreignKey: "assignedToUserId", as: "assignedTo" });
  };
  return Lead;
};