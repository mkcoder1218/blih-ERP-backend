...
const profileSchema = Joi.object({
  ...
  employmentCategory: Joi.string().valid("Managerial", "Non-Managerial").allow("", null).optional(),
  assignedStartTime: Joi.string().valid("08:00", "08:30", "09:00").allow("", null).optional(),
  scheduledWorkDays: Joi.array().items(Joi.number().integer().min(1).max(7)).min(0).max(7).unique().optional(),
  employmentStatus: Joi.string().valid(...EMPLOYMENT_STATUSES).allow("", null).optional(),
  additionalNotes: Joi.string().allow("", null).optional(),
  internshipProgram: Joi.string().max(160).allow("", null).optional(),
  internshipInstitution: Joi.string().max(160).allow("", null).optional(),
  internshipMentorUserId: Joi.string().uuid().allow("", null).optional(),
  internshipExpectedEndDate: Joi.date().iso().allow("", null).optional(),
  internshipStatus: Joi.string().valid("active", "completed", "extended", "terminated").allow("", null).optional(),
  internshipStipendType: Joi.string().valid("paid", "unpaid").allow("", null).optional(),
