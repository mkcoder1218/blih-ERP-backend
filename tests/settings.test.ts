...
describe('Settings & Branding Foundation', () => {
  ...
  describe('Isolation & Public Exposure', () => {
    ...
    it('allows public fetching of designated configuration passing businessId context', async () => {
      // Setup a public key
      await request(app).post('/api/settings').set('Authorization', `Bearer ${adminToken}`).send({ key: 'public_feature', value: true, isPublic: true });
      
      const res = await request(app)
        .get('/api/settings/public?businessId=mock-uuid-if-known'); // Assuming the API looks up the business ID successfully
