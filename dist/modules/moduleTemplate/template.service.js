...
const template_dal_1 = require("./template.dal");
const models_1 = require("../../models");
class TemplateService {
    constructor() {
        this.dal = new template_dal_1.TemplateDAL();
    }
    listAll() {
        return this.dal.findAll({});
    }
    async applyTemplate(businessId, moduleKey, reapply = false) {
        // 1. Validate template exists
        const tpl = await this.dal.findByKey(moduleKey);
        if (!tpl)
            throw new Error(`Template for module '${moduleKey}' not found.`);
        ...
        for (const w of tpl.workflows) {
            const existing = await models_1.db.ApprovalWorkflow.findOne({ where: { businessId, key: w.workflowKey, moduleKey } });
            if (existing && !reapply)
                continue; // Skip duplicate
            let wfId = existing?.id;
            if (!existing) {
                const nw = await models_1.db.ApprovalWorkflow.create({
                ...
                if (w.defaultSteps && Array.isArray(w.defaultSteps)) {
                    for (const step of w.defaultSteps) {
                        await models_1.db.ApprovalStep.create({
        ...
        for (const f of tpl.forms) {
            const existing = await models_1.db.FormDefinition.findOne({ where: { businessId, key: f.formKey, moduleKey } });
            if (existing && !reapply)
                continue;
            let fdId = existing?.id;
            if (!existing) {
                const nf = await models_1.db.FormDefinition.create({
                ...
                if (f.defaultFields && Array.isArray(f.defaultFields)) {
                    for (const field of f.defaultFields) {
                        await models_1.db.FormField.create({
