...
class RecruitmentService {
    async provisionForms(businessId) {
        ...
        for (const t of templates) {
            const existing = await models_1.db.FormDefinition.findOne({
    ...
    async publicApply(jobOpeningId, payload) {
        ...
        let { fullName, firstName, lastName, email, phone, source, cvFileId, ...extraFields } = payload;
        ...
        if (!fullName)
            fullName = email || "Anonymous Applicant";
        ...
        if (!cvFileId) {
            ...
            if (cvFileId &&
                !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(cvFileId)) {
                cvFileId = null;
            }
        }
        ...
        return models_1.db.JobApplication.create({
            ...
            if (cvFileId) {
                try {
                    const asset = await models_1.db.FileAsset.findByPk(cvFileId);
                    if (asset) {
                        await asset.update({
    ...
    async advanceApplicant(id, businessId, stage) {
        ...
        if (stage === "hired") {
            const checkUser = await models_1.db.User.findOne({
