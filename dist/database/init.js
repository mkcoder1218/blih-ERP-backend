...
async function ensureFileAssetPublicUploadSchema() {
    try {
        ...
        const desc = await qi.describeTable("file_assets");
        const col = desc["uploadedByUserId"];
        // If the column already allows null, nothing to do
        if (col && col.allowNull === true)
            return;
        // Drop the FK constraint if it exists (Sequelize names it automatically)
        try {
            await sequelize_1.sequelize.query(`ALTER TABLE "file_assets" DROP CONSTRAINT IF EXISTS "file_assets_uploadedByUserId_fkey";`);
        }
        catch {
...
async function ensureInterviewSkillsSchema() {
    ...
    for (const [col, def] of interviewColsToAdd) {
        if (!interviewDesc[col]) {
            await qi.addColumn("hr_interviews", col, def);
...
async function ensureOfferLettersSchema() {
    try {
        ...
        for (const [col, def] of colsToAdd) {
            if (!desc[col]) {
                await qi.addColumn("offer_letters", col, def);
    ...
    try {
        ...
        if (!hasTable) {
            await qi.createTable("candidate_onboardings", {
                ...
                status: {
                    type: DataTypes.ENUM("PENDING_CANDIDATE_COMPLETION", "IN_PROGRESS", "SUBMITTED_FOR_REVIEW", "COMPLETED", "CANCELLED"),
                    allowNull: false,
        ...
        if (hasOfferLetters) {
            const offerDesc = await qi.describeTable("offer_letters");
...
async function ensureNewModelsSchema() {
    ...
    if (!await tableExists("user_calendar_events")) {
        ...
        await qi.addIndex("user_calendar_events", ["businessId", "employeeUserId"], { name: "user_calendar_events_business_employee_idx" });
        await qi.addIndex("user_calendar_events", ["startAt", "endAt"], { name: "user_calendar_events_range_idx" });
        console.log("user_calendar_events table created.");
    }
    else {
        const desc = await qi.describeTable("user_calendar_events").catch(() => ({}));
        const columns = {
            itemType: { type: DataTypes.STRING(30), allowNull: false, defaultValue: "EVENT" },
            projectId: { type: DataTypes.UUID, allowNull: true },
            projectTaskId: { type: DataTypes.UUID, allowNull: true },
        ...
        for (const [name, def] of Object.entries(columns)) {
            if (!desc[name])
                await qi.addColumn("user_calendar_events", name, def);
        }
    }
    ...
    if (!await tableExists("user_calendar_meeting_requests")) {
        ...
        await qi.addIndex("user_calendar_meeting_requests", ["businessId", "recipientUserId", "status"], { name: "calendar_meetings_recipient_status_idx" });
